...
func split(remaining, paths):
	...
	var split_percent = rng.randf_range(0.3, 0.7) #sets the split percentage
	...
	if (split_horizontal):
		...
		right_child = Branch.new(Vector2i(position.x, position.y + left_height), Vector2i(size.x, size.y - left_height))
	else:
